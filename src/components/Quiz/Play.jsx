...
class Play extends Component {
  ...
  handleDisableButton = () => {
    if (
      ...
      this.state.currentQuestionIndex === this.state.numberOfQuestions
    ) {
      this.setState({
        previousButtonDisabled: true,
      });
  ...
  endGame = () => {
    ...
    const playerStatsJSON = encodeURIComponent(JSON.stringify(playerStats));

    setTimeout(() => {
      window.location.assign(`/play/QuizSummary?stats=${playerStatsJSON}`);
    }, 1000);
